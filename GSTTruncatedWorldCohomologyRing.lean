import Mathlib
import Mathlib.RingTheory.MvPolynomial.Ideal
import GSTUniversalLefschetzCosmology
import GSTGradedWorldAlgebra

/-!
# GST TRUNCATED WORLD COHOMOLOGY RING

The separate axis, grading and Lefschetz laws of a rectangular GST world
come from one commutative operator algebra.

For an A x B world let H be the digit-axis shift and V the carry-axis shift.
They commute and satisfy H^B = 0, V^A = 0.  Hence the polynomial action of
Z[H,V] factors canonically through

    Z[H,V] / (H^B, V^A).

This file constructs that quotient and its representation on WorldCoef A B.
The original 4 x 3 crown is therefore the specialization

    Z[H,V] / (H^3, V^4).

No fixed twelve-cell argument appears in the construction.
-/

set_option maxHeartbeats 20000000
set_option maxRecDepth 1000000

namespace GSTTruncatedWorldCohomologyRing

open GSTWorldCosmology
open GSTUniversalLefschetzCosmology
open GSTGradedWorldAlgebra
open MvPolynomial
open scoped IsMulCommutative

variable (A B : Nat)

/-! ## 1. The commutative operator ring generated by the two GST axes -/

private theorem axis_generators_commute (A B : Nat) :
    ∀ x ∈ ({digitEndo A B, carryEndo A B} :
      Set (Module.End ℤ (WorldCoef A B))),
    ∀ y ∈ ({digitEndo A B, carryEndo A B} :
      Set (Module.End ℤ (WorldCoef A B))),
    x * y = y * x := by
  intro x hx y hy
  simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hx hy
  rcases hx with rfl | rfl <;>
    rcases hy with rfl | rfl <;>
      first
      | rfl
      | exact digit_carry_commute (A := A) (B := B)
      | exact (digit_carry_commute (A := A) (B := B)).symm

/-- The commutative subring generated by the native digit and carry
endomorphisms.  The ambient endomorphism ring need not be commutative. -/
abbrev WorldOperatorRing : Type :=
  ↥(Subring.closure
      ({digitEndo A B, carryEndo A B} :
        Set (Module.End ℤ (WorldCoef A B))))

instance : IsMulCommutative (WorldOperatorRing A B) :=
  Subring.isMulCommutative_closure (axis_generators_commute A B)

noncomputable def hOp : WorldOperatorRing A B :=
  ⟨digitEndo A B,
    Subring.subset_closure (Set.mem_insert _ _)⟩

noncomputable def vOp : WorldOperatorRing A B :=
  ⟨carryEndo A B,
    Subring.subset_closure
      (Set.mem_insert_of_mem _ (Set.mem_singleton _))⟩

@[simp]
theorem hOp_val :
    (hOp A B : Module.End ℤ (WorldCoef A B)) = digitEndo A B :=
  rfl

@[simp]
theorem vOp_val :
    (vOp A B : Module.End ℤ (WorldCoef A B)) = carryEndo A B :=
  rfl

@[simp]
theorem hOp_pow_depth :
    (hOp A B)^B = 0 := by
  apply Subtype.ext
  exact digitEndo_pow_depth (A:=A) (B:=B)

@[simp]
theorem vOp_pow_depth :
    (vOp A B)^A = 0 := by
  apply Subtype.ext
  exact carryEndo_pow_depth (A:=A) (B:=B)

/-! ## 2. Universal polynomial presentation -/

abbrev WorldPoly : Type := MvPolynomial (Fin 2) ℤ

noncomputable def Hpoly : WorldPoly := X (0 : Fin 2)

noncomputable def Vpoly : WorldPoly := X (1 : Fin 2)

/-- The truncation ideal (H^B,V^A). -/
noncomputable def truncIdeal : Ideal WorldPoly :=
  Ideal.span
    ({Hpoly ^ B, Vpoly ^ A} : Set WorldPoly)

/-- Polynomial evaluation at the native GST axis operators. -/
noncomputable def evalWorldPoly :
    WorldPoly →+* WorldOperatorRing A B :=
  eval₂Hom (Int.castRingHom (WorldOperatorRing A B))
    ![hOp A B, vOp A B]

@[simp]
theorem evalWorldPoly_H :
    evalWorldPoly A B Hpoly = hOp A B := by
  unfold evalWorldPoly Hpoly
  rw [eval₂Hom_X']
  simp

@[simp]
theorem evalWorldPoly_V :
    evalWorldPoly A B Vpoly = vOp A B := by
  unfold evalWorldPoly Vpoly
  rw [eval₂Hom_X']
  simp

/-- The native representation kills both truncation generators. -/
theorem truncIdeal_le_ker_evalWorldPoly :
    ∀ a ∈ truncIdeal A B, evalWorldPoly A B a = 0 := by
  intro a ha
  have hgen : ({Hpoly ^ B, Vpoly ^ A} : Set WorldPoly)
      ⊆ RingHom.ker (evalWorldPoly A B) := by
    intro p hp
    simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hp
    rcases hp with rfl | rfl
    · simp [map_pow, evalWorldPoly_H, hOp_pow_depth]
    · simp [map_pow, evalWorldPoly_V, vOp_pow_depth]
  have hle : truncIdeal A B ≤ RingHom.ker (evalWorldPoly A B) := by
    rw [truncIdeal, Ideal.span_le]
    exact hgen
  have hmem := hle ha
  rwa [RingHom.mem_ker] at hmem

/-- The abstract cohomology ring of an A x B GST world. -/
abbrev WorldCohomologyRing : Type :=
  WorldPoly ⧸ truncIdeal A B

/-- Class of the digit-axis generator. -/
noncomputable def H : WorldCohomologyRing A B :=
  Ideal.Quotient.mk (truncIdeal A B) Hpoly

/-- Class of the carry-axis generator. -/
noncomputable def V : WorldCohomologyRing A B :=
  Ideal.Quotient.mk (truncIdeal A B) Vpoly

@[simp]
theorem H_pow_depth :
    (H A B)^B = 0 := by
  apply Ideal.Quotient.eq_zero_iff_mem.2
  exact Ideal.subset_span (Set.mem_insert _ _)

@[simp]
theorem V_pow_depth :
    (V A B)^A = 0 := by
  apply Ideal.Quotient.eq_zero_iff_mem.2
  exact Ideal.subset_span
    (Set.mem_insert_of_mem _ (Set.mem_singleton _))

/-- The polynomial GST action descends canonically through the quotient. -/
noncomputable def worldOperatorHom :
    WorldCohomologyRing A B →+* WorldOperatorRing A B :=
  Ideal.Quotient.lift
    (truncIdeal A B)
    (evalWorldPoly A B)
    (truncIdeal_le_ker_evalWorldPoly A B)

@[simp]
theorem worldOperatorHom_H :
    worldOperatorHom A B (H A B) = hOp A B := by
  simp [worldOperatorHom, H]

@[simp]
theorem worldOperatorHom_V :
    worldOperatorHom A B (V A B) = vOp A B := by
  simp [worldOperatorHom, V]

/-! ## 3. The quotient ring acts on the world coefficient lattice -/

/-- Action of a cohomology-ring element through its native operator
representation. -/
noncomputable def worldAct
    (r : WorldCohomologyRing A B)
    (g : WorldCoef A B) :
    WorldCoef A B :=
  ((worldOperatorHom A B r :
      WorldOperatorRing A B) :
    Module.End ℤ (WorldCoef A B)) g

@[simp]
theorem worldAct_zero (g : WorldCoef A B) :
    worldAct A B 0 g = 0 := by
  simp [worldAct]

@[simp]
theorem worldAct_one (g : WorldCoef A B) :
    worldAct A B 1 g = g := by
  simp [worldAct]

theorem worldAct_add
    (r s : WorldCohomologyRing A B)
    (g : WorldCoef A B) :
    worldAct A B (r+s) g =
      fun c => worldAct A B r g c + worldAct A B s g c := by
  funext c
  show worldAct A B (r+s) g c
    = worldAct A B r g c + worldAct A B s g c
  unfold worldAct
  rw [map_add]
  rfl

theorem worldAct_mul
    (r s : WorldCohomologyRing A B)
    (g : WorldCoef A B) :
    worldAct A B (r*s) g =
      worldAct A B r (worldAct A B s g) := by
  unfold worldAct
  rw [map_mul]
  rfl

@[simp]
theorem worldAct_H (g : WorldCoef A B) :
    worldAct A B (H A B) g = digitShiftN 1 g := by
  simp [worldAct, hOp]

@[simp]
theorem worldAct_V (g : WorldCoef A B) :
    worldAct A B (V A B) g = carryShiftN 1 g := by
  simp [worldAct, vOp]

private theorem worldAct_H_pow (m : Nat) :
    ∀ x : WorldCoef A B,
    worldAct A B ((H A B)^m) x = digitShiftN m x := by
  induction m with
  | zero =>
      intro x
      rw [pow_zero, worldAct_one, digitShiftN_zero]
  | succ m ih =>
      intro x
      rw [pow_succ, worldAct_mul, worldAct_H, ih, digitShiftN_add]

private theorem worldAct_V_pow (n : Nat) :
    ∀ x : WorldCoef A B,
    worldAct A B ((V A B)^n) x = carryShiftN n x := by
  induction n with
  | zero =>
      intro x
      rw [pow_zero, worldAct_one, carryShiftN_zero]
  | succ n ih =>
      intro x
      rw [pow_succ, worldAct_mul, worldAct_V, ih, carryShiftN_add]

/-- Every monomial H^m V^n acts by the corresponding mixed native shift. -/
theorem worldAct_monomial
    (m n : Nat) (g : WorldCoef A B) :
    worldAct A B ((H A B)^m * (V A B)^n) g =
      digitShiftN m (carryShiftN n g) := by
  rw [worldAct_mul, worldAct_V_pow A B n g, worldAct_H_pow A B m]

/-- Mixed monomials crossing either rectangle boundary act as zero. -/
theorem worldAct_monomial_zero
    (m n : Nat) (g : WorldCoef A B)
    (h : B ≤ m ∨ A ≤ n) :
    worldAct A B ((H A B)^m * (V A B)^n) g = 0 := by
  rw [worldAct_monomial]
  exact mixed_boundary_extinction g (by
    rcases h with hm | hn
    · exact Or.inr hm
    · exact Or.inl hn)

/-! ## 4. Lefschetz element and recovery of the old crown -/

/-- The universal Lefschetz class L = H+V. -/
noncomputable def L : WorldCohomologyRing A B :=
  H A B + V A B

@[simp]
theorem worldAct_L (g : WorldCoef A B) :
    worldAct A B (L A B) g = lefschetzEndo A B g := by
  have hsplit : worldAct A B (L A B) g
      = worldAct A B (H A B) g + worldAct A B (V A B) g := by
    unfold worldAct L
    rw [map_add]
    rfl
  rw [hsplit, worldAct_H, worldAct_V]
  rfl

/-- The universal Lefschetz class is nilpotent at the sharp rectangular
ceiling inherited from the native operator action. -/
theorem worldAct_L_pow_boundary
    (g : WorldCoef A B) :
    worldAct A B ((L A B)^(A+B-1)) g = 0 := by
  change
    ((worldOperatorHom A B ((L A B)^(A+B-1)) :
      WorldOperatorRing A B) :
      Module.End ℤ (WorldCoef A B)) g = 0
  rw [map_pow]
  have hmem : lefschetzEndo A B ∈
      Subring.closure
        ({digitEndo A B, carryEndo A B} :
          Set (Module.End ℤ (WorldCoef A B))) := by
    rw [lefschetzEndo]
    exact Subring.add_mem _
      (Subring.subset_closure (Set.mem_insert _ _))
      (Subring.subset_closure
        (Set.mem_insert_of_mem _ (Set.mem_singleton _)))
  have hL :
      worldOperatorHom A B (L A B) =
        ⟨lefschetzEndo A B, hmem⟩ := by
    apply Subtype.ext
    show (worldOperatorHom A B (H A B + V A B) :
        Module.End ℤ (WorldCoef A B))
      = lefschetzEndo A B
    rw [map_add, worldOperatorHom_H, worldOperatorHom_V]
    rfl
  rw [hL]
  have hz :
      (lefschetzEndo A B)^(A+B-1) = 0 :=
    lefschetz_pow_boundary (A:=A) (B:=B)
  have hz' :
      (⟨lefschetzEndo A B, hmem⟩ :
        WorldOperatorRing A B)^(A+B-1) = 0 := by
    apply Subtype.ext
    exact hz
  rw [hz']
  rfl

/-- The historical HC chart is exactly the quotient Z[H,V]/(H^3,V^4). -/
abbrev HCCohomologyRing : Type :=
  WorldCohomologyRing 4 3

theorem hc_H_cube :
    (H 4 3)^3 = 0 :=
  H_pow_depth 4 3

theorem hc_V_fourth :
    (V 4 3)^4 = 0 :=
  V_pow_depth 4 3

theorem hc_L_six_acts_zero
    (g : WorldCoef 4 3) :
    worldAct 4 3 ((L 4 3)^6) g = 0 := by
  simpa using worldAct_L_pow_boundary 4 3 g

/-- One capstone: the rectangular GST world is represented by a canonical
truncated two-generator commutative cohomology ring. -/
theorem truncated_world_ring_crown :
    (∀ A B, (H A B)^B = 0)
    ∧ (∀ A B, (V A B)^A = 0)
    ∧ (∀ A B (g : WorldCoef A B),
      worldAct A B (H A B) g = digitShiftN 1 g)
    ∧ (∀ A B (g : WorldCoef A B),
      worldAct A B (V A B) g = carryShiftN 1 g)
    ∧ (∀ A B (g : WorldCoef A B),
      worldAct A B ((L A B)^(A+B-1)) g = 0) := by
  exact ⟨
    fun A B => H_pow_depth A B,
    fun A B => V_pow_depth A B,
    fun A B g => worldAct_H A B g,
    fun A B g => worldAct_V A B g,
    fun A B g => worldAct_L_pow_boundary A B g⟩

#check WorldOperatorRing
#check H
#check V
#check L
#check H_pow_depth
#check V_pow_depth
#check worldOperatorHom
#check worldAct_H
#check worldAct_V
#check worldAct_monomial
#check worldAct_monomial_zero
#check worldAct_L
#check worldAct_L_pow_boundary
#check HCCohomologyRing
#check truncated_world_ring_crown

#print axioms truncIdeal_le_ker_evalWorldPoly
#print axioms worldAct_monomial
#print axioms worldAct_L_pow_boundary
#print axioms truncated_world_ring_crown


/-! ## Universal coordinate geometry and its inverse system of quotients -/
/-- Every finite geometry is projected from the same two-generator algebra. -/
noncomputable def coordinateProjection : WorldPoly →+* WorldCohomologyRing A B :=
  Ideal.Quotient.mk (truncIdeal A B)

private theorem restriction_kernel {C D : ℕ} (hA : A ≤ C) (hB : B ≤ D) :
    ∀ a ∈ truncIdeal C D, coordinateProjection A B a = 0 := by
  intro a ha
  have hgen : ({Hpoly ^ D, Vpoly ^ C} : Set WorldPoly)
      ⊆ RingHom.ker (coordinateProjection A B) := by
    intro p hp
    simp only [Set.mem_insert_iff, Set.mem_singleton_iff] at hp
    rcases hp with rfl | rfl
    · simp [map_pow]
      exact pow_eq_zero_of_le hB (H_pow_depth A B)
    · simp [map_pow]
      exact pow_eq_zero_of_le hA (V_pow_depth A B)
  have hle : truncIdeal C D ≤ RingHom.ker (coordinateProjection A B) := by
    rw [truncIdeal, Ideal.span_le]
    exact hgen
  have hmem := hle ha
  rwa [RingHom.mem_ker] at hmem

/-- Enlarging a window weakens its ideal; restriction is a genuine ring map. -/
noncomputable def quotientRestriction {C D : ℕ} (hA : A ≤ C) (hB : B ≤ D) :
    WorldCohomologyRing C D →+* WorldCohomologyRing A B :=
  Ideal.Quotient.lift (truncIdeal C D) (coordinateProjection A B)
    (restriction_kernel A B hA hB)

@[simp] theorem quotientRestriction_projection {C D : ℕ}
    (hA : A ≤ C) (hB : B ≤ D) (p : WorldPoly) :
    quotientRestriction A B hA hB (coordinateProjection C D p) =
      coordinateProjection A B p := rfl

theorem quotientRestriction_trans {C D E F : ℕ}
    (hAC : A ≤ C) (hBD : B ≤ D) (hCE : C ≤ E) (hDF : D ≤ F)
    (x : WorldCohomologyRing E F) :
    quotientRestriction A B hAC hBD (quotientRestriction C D hCE hDF x) =
      quotientRestriction A B (hAC.trans hCE) (hBD.trans hDF) x := by
  refine Quotient.inductionOn x ?_
  intro p
  rfl

/-- Formal coordinate completion is explicitly a compatible family of
quotient geometries; no unsupported identification with algebraic fields. -/
def FormalCoordinateCosmos :=
  {x : ∀ A B, WorldCohomologyRing A B //
    ∀ A B C D (hA : A ≤ C) (hB : B ≤ D),
      quotientRestriction A B hA hB (x C D) = x A B}

noncomputable def polynomialCompletion (p : WorldPoly) : FormalCoordinateCosmos :=
  ⟨fun A B => coordinateProjection A B p, by intros; rfl⟩


/-- Exact generic support criterion for the window ideal: every monomial
lies beyond at least one of the two observation depths. -/
theorem mem_truncIdeal_iff (p : WorldPoly) :
    p ∈ truncIdeal A B ↔ ∀ d ∈ p.support, B ≤ d 0 ∨ A ≤ d 1 := by
  have h := (MvPolynomial.mem_ideal_span_monomial_image
    (x:=p) (s:=({Finsupp.single (0 : Fin 2) B,
      Finsupp.single (1 : Fin 2) A} : Set (Fin 2 →₀ ℕ))))
  simpa [truncIdeal, Hpoly, Vpoly, Set.image_insert_eq, Set.image_singleton,
    ← MvPolynomial.X_pow_eq_monomial, Finsupp.single_le_iff] using h

/-- No nonzero polynomial disappears in every finite quotient geometry. -/
theorem coordinateProjections_separate {p q : WorldPoly}
    (h : ∀ A B, coordinateProjection A B p = coordinateProjection A B q) : p = q := by
  apply sub_eq_zero.mp
  apply MvPolynomial.ext
  intro d
  rw [MvPolynomial.coeff_zero]
  by_contra hd
  have hs : d ∈ (p-q).support := MvPolynomial.mem_support_iff.mpr hd
  have hi : p-q ∈ truncIdeal (d 1 + 1) (d 0 + 1) := by
    apply Ideal.Quotient.eq_zero_iff_mem.mp
    change coordinateProjection (d 1 + 1) (d 0 + 1) (p-q) = 0
    rw [map_sub, h, sub_self]
  have hb := (mem_truncIdeal_iff (d 1 + 1) (d 0 + 1) (p-q)).mp hi d hs
  omega

/-- Passing from algebraic coordinates to formal observations loses no
polynomial information; surjectivity is deliberately not asserted. -/
theorem polynomialCompletion_injective : Function.Injective polynomialCompletion := by
  intro p q h
  apply coordinateProjections_separate
  intro A B
  exact congrArg (fun x : FormalCoordinateCosmos => x.val A B) h

#print axioms coordinateProjections_separate
#print axioms polynomialCompletion_injective

end GSTTruncatedWorldCohomologyRing

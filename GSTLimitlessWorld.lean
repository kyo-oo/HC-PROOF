...
As an additive monoid algebra this is simultaneously a finitely supported
field on cells and the free commutative integer algebra generated by the two
...
theorem restrictWorld_extendWorld {A B : Nat} (g : WorldCoef A B) :
  ...
  funext c
  simp only [restrictWorld, extendWorld, Finset.sum_apply]
...
theorem supportInside_extendWorld {A B : Nat} (g : WorldCoef A B) :
  ...
  by_contra hout
  push_neg at hout
  have hzero : extendWorld g c = 0 := by
    simp only [extendWorld, Finset.sum_apply]

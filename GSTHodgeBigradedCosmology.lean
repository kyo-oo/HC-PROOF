...
/-- **MULTIPLICITY-FREE BIGRADING.**
Degree and charge together determine a world cell uniquely. -/
theorem worldHodgeBidegree_injective
    {A B : Nat} :
...
theorem hodgeBidegreeCell_subsingleton
    (A B k : Nat) (q : Int) :
...
theorem worldHodgeBidegreeProj_apply
    {A B : Nat} (k : Nat) (q : Int)
...
theorem worldHodgeBidegreeProj_idempotent
    {A B : Nat} (k : Nat) (q : Int)
    (f : WorldCoef A B) :
    worldHodgeBidegreeProj k q
      (worldHodgeBidegreeProj k q f) =
        worldHodgeBidegreeProj k q f := by
  funext c
  rw [worldHodgeBidegreeProj_apply,
      worldHodgeBidegreeProj_apply]
  by_cases h : worldHodgeBidegree c = (k,q) <;> simp [h]

/-- Distinct joint bidegrees are orthogonal. -/
theorem worldHodgeBidegreeProj_orthogonal
    {A B : Nat}
    (k j : Nat) (q r : Int)
    (hneq : (k,q) ≠ (j,r))
    (f : WorldCoef A B) :
    worldHodgeBidegreeProj k q
      (worldHodgeBidegreeProj j r f) = fun _ => 0 := by
  funext c
  rw [worldHodgeBidegreeProj_apply,
      worldHodgeBidegreeProj_apply]
...
theorem digitShiftN_respects_bidegree
    {A B : Nat} (n k : Nat) (q : Int)
    (f : WorldCoef A B) :
    digitShiftN n (worldHodgeBidegreeProj k q f) =
      worldHodgeBidegreeProj (k+n) (q-(n:Int))
        (digitShiftN n f) := by
...
theorem carryShiftN_respects_bidegree
    {A B : Nat} (n k : Nat) (q : Int)
    (f : WorldCoef A B) :
...
theorem worldHodgeBidegree_dual
...
def hodgeBidegreeDualEquiv
    {A B : Nat} (k : Nat) (q : Int)
    (hk : k ≤ A+B-2) :
    HodgeBidegreeCell A B k q ≃
      HodgeBidegreeCell A B (A+B-2-k)
        (worldChargeCenter A B-q) where
  toFun c :=
    ⟨worldDual c.1, by
...
theorem diagonalState_bidegree
    {A B p : Nat} (hpA : p < A) (hpB : p < B) :
    worldHodgeBidegree
      (diagonalState hpA hpB) = (2*p,0) := by
  unfold worldHodgeBidegree worldDegree worldHodgeCharge diagonalState
  simp
  constructor <;> omega

/-- A live weight-p Hodge class is exactly a fixed vector of the single
bidegree projector (2p,0). -/
theorem worldHodgeClass_iff_bidegree_fixed
    {A B p : Nat} (hpA : p < A) (hpB : p < B)
    (f : ShapeCoef (outputShape A B)) :
    isWorldHodgeClass p f ↔
      worldHodgeBidegreeProj (2*p) 0 f = f := by
  constructor
  · intro hf
    funext c
...
/-- Capstone: multiplicity-free bidegrees, exact axis motion, Poincare
reflection and identification of the Hodge diagonal all coexist. -/
theorem hodge_bigraded_cosmology_crown :
    ...
    ∧ (∀ A B (c : WorldCell A B),
      worldHodgeBidegree (worldDual c) =
        (A+B-2-worldDegree c,
          worldChargeCenter A B-worldHodgeCharge c)) := by
  refine ⟨?_, digitShiftN_respects_bidegree,
    carryShiftN_respects_bidegree, worldHodgeBidegree_dual⟩
  intro A B

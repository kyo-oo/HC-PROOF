...
/-! ## Closed affine-channel dynamics -/

/-- Every physical channel state remains inside the four-state sector after
reading one physical ternary digit. -/
theorem channelNext_lt_four
    (c a : Nat) (hc : c < 4) (ha : a < 3) :
    channelNext c a < 4 := by
  interval_cases c <;> interval_cases a <;>
    norm_num [channelNext] at *

/-- One complete channel transition, retaining both the next finite state and
the unread ternary tail. -/
def channelStep (c x : Nat) : Nat × Nat :=
  (channelNext c (lowDigit x), tail3 x)

/-- Iterated affine-channel dynamics on the exact ternary tail. -/
def channelOrbit : Nat → Nat → Nat → Nat × Nat
  | 0, c, x => (c, x)
  | n+1, c, x =>
      channelOrbit n (channelNext c (lowDigit x)) (tail3 x)

/-- **CHANNEL CLOSURE.**  Starting in a physical channel, every finite iterate
remains in the same four-state channel universe, regardless of source length. -/
theorem channelOrbit_state_lt_four
    (n c x : Nat) (hc : c < 4) :
    (channelOrbit n c x).1 < 4 := by
  induction n generalizing c x with
  | zero =>
      simpa [channelOrbit] using hc
  | succ n ih =>
      rw [channelOrbit]
      exact ih _ _
        (channelNext_lt_four c (lowDigit x) hc (lowDigit_lt_three x))

/-- The unread source after one transition is literally the ternary quotient. -/
@[simp] theorem channelStep_tail (c x : Nat) :
    (channelStep c x).2 = x / 3 := by
  rfl

/-- The finite channel component of one transition is the exact arithmetic
carry generated by the low source trit. -/
@[simp] theorem channelStep_state (c x : Nat) :
    (channelStep c x).1 = (4 * (x % 3) + c) / 3 := by
  rfl
...
#check channelNext_lt_four
#check channelOrbit_state_lt_four
...
#print axioms channelOrbit_state_lt_four
